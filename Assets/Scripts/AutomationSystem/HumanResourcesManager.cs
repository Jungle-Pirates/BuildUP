...
{
    public HumanResourceOffice humanResourceOffice; // 인력 사무소 참조
    ...
    {
        int previousTotal = humanResourceOffice.totalWorkers;
        humanResourceOffice.totalWorkers = 0;

        // 모든 주거지 클러스터에서 제공하는 일꾼 수 계산
        foreach (int workers in clusterTotalWorkers)
        {
            humanResourceOffice.totalWorkers += workers;
        }

        // 새로 획득한/잃은 일꾼 수 계산
        int workerDifference = humanResourceOffice.totalWorkers - previousTotal;

        // 가용 일꾼 수 업데이트
        humanResourceOffice.availableWorkers += workerDifference;

        // 가용 일꾼이 음수가 되면 방에서 일꾼 회수
        if (humanResourceOffice.availableWorkers < 0)
        {
            RecoverWorkersFromRooms(-humanResourceOffice.availableWorkers);
        }
    ...
    {
        if (humanResourceOffice.availableWorkers >= count)
        {
            if (room.AssignWorker(count))
            {
                humanResourceOffice.availableWorkers -= count;
    ...
    private void RecoverWorkersFromRooms(int count)
    {
        ...
        */
        ...
        humanResourceOffice.availableWorkers = 0;
    }
